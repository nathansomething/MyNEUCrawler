...
public class Crawler {
	...
	List<Course> getCourses(String season, String year) {
		List<Course> courses = new ArrayList<>();
		Integer numSubjects = 2;//this.getNumberSubjects(season, year);
		for (Integer counter = 1; counter < numSubjects; counter++) { //-1 to account for small table at the end
			this.goToClassSchedule();
			this.selectTerm(season, year);
			WebElement submitButton = this.driver.findElement(By.cssSelector("input[value=Submit]"));
			WebElement subjectMenu = this.driver.findElement(By.id("subjct_id"));
			WebElement all = subjectMenu.findElement(By.cssSelector("option:nth-child(1)"));
			WebElement subject = subjectMenu.findElement(By.cssSelector("option:nth-child(" + (counter + 1) + ")"));
			all.click();
			subject.click();
			submitButton.click();
